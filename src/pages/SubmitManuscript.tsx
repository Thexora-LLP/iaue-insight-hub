...
const SubmitManuscript = () => {
  ...
  // Checkbox states for review step
  ...
  const validateStep2 = () => {
    return manuscriptFile !== null && coverLetterFile !== null;
  };
  ...
  const validateStep3 = () => {
    return authors.every(author => 
      ...
      /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(author.email) // Basic email validation
    );
  };
  ...
  const validateStep4 = () => {
    return (
      ...
      manuscriptDetails.abstract.trim() !== "" &&
      manuscriptDetails.keywords.trim() !== "" &&
      manuscriptDetails.category !== ""
    );
  };

  const validateStep5 = () => {
    return agreements.terms && agreements.copyright;
  };
  ...
  const getValidationMessage = (step) => {
    switch (step) {
      case 1:
        if (!selectedType) return "Please select a manuscript type";
        return "";
      case 2:
        if (!manuscriptFile) return "Main manuscript file is required";
        if (!coverLetterFile) return "Cover letter file is required";
        return "";
      case 3:
        const invalidAuthor = authors.find(author => 
          !author.name.trim() || !author.email.trim() || !author.affiliation.trim() ||
          !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(author.email)
        );
        if (invalidAuthor) return "Please fill all required author fields with valid information";
        return "";
      case 4:
        if (!manuscriptDetails.title.trim()) return "Manuscript title is required";
        if (!manuscriptDetails.abstract.trim()) return "Abstract is required";
        if (!manuscriptDetails.keywords.trim()) return "Keywords are required";
        if (!manuscriptDetails.category) return "Subject category is required";
        return "";
      case 5:
        if (!agreements.terms) return "Please agree to the terms and conditions";
        if (!agreements.copyright) return "Please confirm author contributions and agreement";
        return "";
      default:
        return "";
    }
  ...
  const updateManuscriptDetails = (field, value) => {
          ...
          <div className="space-y-6">
            ...
            {!validateStep1() && (
              <div className="flex items-center gap-2 text-amber-600 bg-amber-50 p-3 rounded-md">
                ...
                <span className="text-sm">{getValidationMessage(1)}</span>
              </div>
            )}
          </div>
        );
        ...
        return (
          <div className="space-y-6">
            ...
            <FileUploadCard
              ...
              multiple={true}
              description="Upload figures, datasets, or supplementary materials"
              onFileUpload={(files) => setAdditionalFiles(files)}
              onFileRemove={() => setAdditionalFiles([])}
            />
            ...
            {!validateStep2() && (
              <div className="flex items-center gap-2 text-amber-600 bg-amber-50 p-3 rounded-md">
                <AlertCircle className="h-4 w-4" />
                <span className="text-sm">{getValidationMessage(2)}</span>
              </div>
            )}
          </div>
        );
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => updateAuthor(index, "name", e.target.value)}
                          placeholder="Dr. John Doe"
                          className={!author.name.trim() ? "border-red-300 focus:border-red-500" : ""}
                        />
                      </div>
                      <div>
                        <Label htmlFor={`email-${index}`}>Email Address *</Label>
                        <Input
                          ...
                          onChange={(e) => updateAuthor(index, "email", e.target.value)}
                          placeholder="john.doe@university.edu"
                          className={!author.email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(author.email) ? "border-red-300 focus:border-red-500" : ""}
                        />
                      </div>
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateAuthor(index, "affiliation", e.target.value)}
                        placeholder="Department, University, Country"
                        className={!author.affiliation.trim() ? "border-red-300 focus:border-red-500" : ""}
                      />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
              ...
              {!validateStep3() && (
                <div className="flex items-center gap-2 text-amber-600 bg-amber-50 p-3 rounded-md">
                  <AlertCircle className="h-4 w-4" />
                  <span className="text-sm">{getValidationMessage(3)}</span>
                </div>
              )}
            </div>
          </div>
        ...
        return (
          <div className="space-y-6">
              ...
              <CardContent className="p-6 space-y-6">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateManuscriptDetails("title", e.target.value)}
                    placeholder="Enter the full title of your manuscript"
                    className={`mt-1 ${!manuscriptDetails.title.trim() ? "border-red-300 focus:border-red-500" : ""}`}
                  />
                </div>
                ...
                <div>
                  ...
                  <Textarea
                    id="abstract"
                    value={manuscriptDetails.abstract}
                    onChange={(e) => updateManuscriptDetails("abstract", e.target.value)}
                    placeholder="Provide a comprehensive abstract (150-300 words)"
                    className={`mt-1 min-h-[120px] ${!manuscriptDetails.abstract.trim() ? "border-red-300 focus:border-red-500" : ""}`}
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    id="keywords"
                    value={manuscriptDetails.keywords}
                    onChange={(e) => updateManuscriptDetails("keywords", e.target.value)}
                    placeholder="Enter 5-8 keywords separated by commas"
                    className={`mt-1 ${!manuscriptDetails.keywords.trim() ? "border-red-300 focus:border-red-500" : ""}`}
                  />
                </div>

                <div>
                  <Label htmlFor="category">Subject Category *</Label>
                  <Select value={manuscriptDetails.category} onValueChange={(value) => updateManuscriptDetails("category", value)}>
                    <SelectTrigger className={`mt-1 ${!manuscriptDetails.category ? "border-red-300" : ""}`}>
                      <SelectValue placeholder="Select a category" />
                    </SelectTrigger>
                    <SelectContent>
                      {categories.map((category) => (
                ...
                <div>
                  ...
                  <Textarea
                    id="funding"
                    value={manuscriptDetails.funding}
                    onChange={(e) => updateManuscriptDetails("funding", e.target.value)}
                    placeholder="List funding sources, grant numbers, and acknowledgments"
                    className="mt-1"
                  />
                </div>
            ...
            {!validateStep4() && (
              <div className="flex items-center gap-2 text-amber-600 bg-amber-50 p-3 rounded-md">
                <AlertCircle className="h-4 w-4" />
                <span className="text-sm">{getValidationMessage(4)}</span>
              </div>
            )}
          </div>
        );
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Type</p>
                    <p className="text-foreground">{manuscriptTypes.find(t => t.id === selectedType)?.title || "Not selected"}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Title</p>
                    <p className="text-foreground">{manuscriptDetails.title || "Not provided"}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Authors</p>
                    <p className="text-foreground">
                      {authors.map(author => `${author.name}${author.isCorresponding ? ' (Corresponding)' : ''}`).join(', ') || "Not provided"}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Category</p>
                    <p className="text-foreground">{categories.find(c => c.toLowerCase().replace(/\s+/g, '-') === manuscriptDetails.category) || "Not selected"}</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <CardContent className="p-6">
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => updateAgreement("copyright", checked)}
                  />
                  <Label htmlFor="copyright" className="text-sm">
                    I confirm that all authors have contributed significantly and agree to the submission
                  </Label>
            ...
            {!validateStep5() && (
              <div className="flex items-center gap-2 text-amber-600 bg-amber-50 p-3 rounded-md">
                <AlertCircle className="h-4 w-4" />
                <span className="text-sm">{getValidationMessage(5)}</span>
              </div>
            )}
          </div>
        );
