...
const mockUser: User = { id: "u-1", name: "Jane Doe", email: "jane.doe@example.com", role: "user" };
...
export async function register(name: string, email: string, password: string): Promise<{ userId: string }>{
  await delay();
...
export async function resetPassword(token: string, newPassword: string): Promise<{ ok: true }>{
  await delay();
  if (!token || !newPassword) return Promise.reject({ message: "Invalid request" });
  return { ok: true };
}
